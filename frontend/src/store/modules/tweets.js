...
export default {
    ...
    actions: {
        ...
        startListener({ commit }, userinfo) {
            ...
            // firestoreからデータを検索する
            this.unsubscribe = LinkRef.where('account', '==', userinfo.screenName).orderBy("created_at", "desc").limit(50).onSnapshot(querySnapshot => {
                    // データが更新されるたびに呼び出される
        ...
        startListenerAll({ commit }) {
            ...
            // firestoreからデータを検索する
            this.unsubscribe = LinkRef.orderBy("created_at", "desc").limit(50).onSnapshot(querySnapshot => {
                    // データが更新されるたびに呼び出される
                    querySnapshot.docChanges().some(change => {
                        // 時刻がnullのものとログインユーザー以外は表示しない
                        if (!change.doc.data().account || !change.doc.data().created_at || change.doc.data().checkbox == true) {
                            return
                        }
                        // ステート更新するために配列に格納（DBから直接読み込むと同期が追いつかない）
                        const payload = {
